...
describe("Bootcamp", async () => {
  ...
  before(async (): Promise<any> => {
    ...
    const Course = await ethers.getContractFactory("Course", deployer);
    course = await Course.deploy();
    await course.deployed();

    const Bootcamp = await ethers.getContractFactory("Bootcamp", signer1);
  ...
  it("Should emit Cloned course address", async () => {
    const clonedContractAddress = '0x' + keccak256(ethers.utils.RLP.encode([bootcamp.address, '0x01']))
        .toString('hex').slice(-40);
    const clonedContractAddressWithChecksum = ethers.utils.getAddress(clonedContractAddress);
    await expect(bootcamp.createCourse(CID)).to.emit(bootcamp, "CourseCreated")
