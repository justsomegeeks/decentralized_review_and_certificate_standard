 ...
 describe("Course", async () => {
     ...
     before(async () => {
         ...
         await course.initialize(CID, deployer.getAddress());

         students = await ethers.provider.listAccounts();
         merkleTree = new MerkleTree(students, keccak256, { hashLeaves: true, sortPairs: true });
     ...
     it("Should emit graduate event", async () => {
         await expect(course.graduate(root, CID)).to.emit(course, "Graduate")
             .withArgs(root, CID);
     })
